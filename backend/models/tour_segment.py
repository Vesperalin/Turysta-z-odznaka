...
class Tour_segment(db.Model, BaseModel):
    ...
    tour_id = db.Column('FK_trasy', db.Integer, db.ForeignKey('trasy.id_trasy'), nullable=False)
    labeled_segment_id = db.Column('FK_odcinekOpisany', db.Integer, db.ForeignKey('odcinkiopisane.id_odcinka'), nullable=True)
    # own_segment_id simplification - TOUR HAS ONLY LABELED SEGMENTS
    tour = db.relationship('Tour', backref="tour_segments")
    labeled_segment = db.relationship('Labeled_segment', backref="tour_segments")
